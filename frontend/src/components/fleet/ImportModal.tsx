...
import { downloadTemplate, parseImportFile, type ImportResult } from '@/lib/fleet/excel'
...
export default function ImportModal({
  ...
  function reset() {
    setFileName(''); setParsed(null); setDone(null); setBusy(false)
  }
  ...
  async function onFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    setDone(null)
    try {
      const result = await parseImportFile(file, defaultBranch)
      setParsed(result)
    } catch {
  ...
  function commit() {
    if (!parsed) return
    // Skip rows that clash with existing fleet numbers or plates.
    const toAdd = []
    let skipped = 0
    for (const r of parsed.valid) {
      if (vehiclesStore.conflict(r.fleet_no, r.reg_plate)) {
        skipped++
        continue
      }
      toAdd.push(r)
    }
    vehiclesStore.bulkAdd(toAdd)
  ...
  return (
    <Modal
      ...
      subtitle="Import from an Excel (.xlsx) file. Existing fleet numbers are skipped."
      footer={
        done ? (
          ...
          <>
            ...
            <Button onClick={commit} disabled={!parsed || parsed.valid.length === 0}>
              Import {parsed ? parsed.valid.length : 0} vehicle{parsed?.valid.length === 1 ? '' : 's'}
            </Button>
          </>
        )
      }
    >
      ...
      <div className="mb-4 flex items-center justify-between rounded-lg bg-canvas px-4 py-3">
        <div className="text-sm text-navy">
          ...
          <div className="text-xs text-status-neutral">Same columns the import expects.</div>
        </div>
      ...
      {parsed && !busy && (
        <div className="mt-4 space-y-3">
          <div className="flex items-center gap-2 rounded-lg border border-status-good/30 bg-status-good/5 px-3 py-2 text-sm text-status-good">
            ...
            {parsed.valid.length} vehicle{parsed.valid.length === 1 ? '' : 's'} ready to import
          </div>
          ...
          {parsed.errors.length > 0 && (
            <div className="rounded-lg border border-status-critical/30 bg-status-critical/5 px-3 py-2 text-sm text-status-critical">
              <div className="mb-1 flex items-center gap-2 font-medium">
                <AlertTriangle size={16} /> {parsed.errors.length} row(s) skipped
              </div>
            ...
            <div className="max-h-44 overflow-auto rounded-lg border border-black/10">
              <table className="w-full text-left text-xs">
                <thead className="sticky top-0 bg-navy text-white">
                  <tr>
                    <th className="px-3 py-1.5 font-medium">Fleet No</th>
                    <th className="px-3 py-1.5 font-medium">Plate</th>
                ...
                <tbody>
                  {parsed.valid.slice(0, 50).map((v, i) => (
                    <tr key={i} className="border-t border-black/5">
                      <td className="px-3 py-1.5 font-medium text-navy">{v.fleet_no}</td>
                      <td className="px-3 py-1.5">{v.reg_plate}</td>
                      <td className="px-3 py-1.5">{v.make} {v.model}</td>
                      <td className="px-3 py-1.5 capitalize">{v.branch}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      {done && (
        <div className="mt-4 flex flex-col items-center gap-2 rounded-xl bg-canvas px-6 py-8 text-center">
          ...
          <div className="text-sm text-status-neutral">
            {done.added} added{done.skipped > 0 && `, ${done.skipped} skipped (duplicates or errors)`}.
          </div>
        </div>
      )}
    </Modal>
