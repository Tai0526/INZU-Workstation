...
export default function Dashboard() {
  ...
  const allItems = useMemo<Item[]>(() => {
    ...
    if (real.licExpired > 0)
      push({ id: 'lic', severity: 'critical', icon: FileWarning, title: `${real.licExpired} licensing ${plural(real.licExpired, 'item')} expired`, detail: 'Vehicles legally need a renewed document on file.', link: '/fleet/licensing?filter=noncompliant', actors: [...OPS, 'workshop_supervisor'] })
    if (real.licExpiring > 0)
      push({ id: 'lic-soon', severity: 'warning', icon: FileWarning, title: `${real.licExpiring} licensing ${plural(real.licExpiring, 'item')} expiring soon`, detail: 'Renew before they lapse to stay road-legal.', link: '/fleet/licensing?filter=expiring', actors: [...OPS, 'workshop_supervisor'] })
    if (real.grounded > 0)
      push({ id: 'ground', severity: 'warning', icon: Truck, title: `${real.grounded} ${plural(real.grounded, 'vehicle')} grounded`, detail: 'Out of service — reducing available capacity.', link: '/fleet/vehicles?status=grounded', actors: [...OPS, 'workshop_supervisor'] })
    if (real.missingDocs > 0)
      push({ id: 'missing-docs', severity: 'warning', icon: FileWarning, title: `${real.missingDocs} ${plural(real.missingDocs, 'vehicle')} missing required documents`, detail: 'Workshop to upload road tax, fitness, insurance or FQM inspection.', link: '/fleet/licensing?filter=noncompliant', actors: ['workshop_supervisor', ...OPS] })

    // Incidents (real)
    if (r.incOps > 0)
      push({ id: 'inc-ops', severity: 'critical', icon: ShieldAlert, title: `${r.incOps} ${plural(r.incOps, 'verdict')} awaiting your decision`, detail: 'Safety proposed a verdict — approve or reject it.', link: '/safety/incidents?stage=ops_review', actors: [...OPS, 'hr_manager'] })
    if (r.incSafety > 0)
    ...
    if (r.confirmedToEscalate > 0)
      push({ id: 'speed', severity: 'action', icon: Gauge, title: `${r.confirmedToEscalate} confirmed speeding ${plural(r.confirmedToEscalate, 'event')} to escalate`, detail: 'Raise an incident so Safety can act.', link: '/speed/events?status=confirmed', actors: SPEED })

    // Hazards (real)
    if (r.highHaz > 0)
      push({ id: 'haz-high', severity: 'critical', icon: Flame, title: `${r.highHaz} high-risk ${plural(r.highHaz, 'hazard')} open`, detail: 'Severity × likelihood ≥ 10 — control these first.', link: '/safety/hazards?status=open', actors: SAFETY })
    else if (r.openHaz > 0)
      push({ id: 'haz', severity: 'warning', icon: Flame, title: `${r.openHaz} open ${plural(r.openHaz, 'hazard')}`, detail: 'Near misses / unsafe conditions awaiting close-out.', link: '/safety/hazards?status=open', actors: SAFETY })

    // CAP, compliance (real)
    if (r.capOpen > 0)
      push({ id: 'cap', severity: 'warning', icon: ClipboardCheck, title: `${r.capOpen} CAP ${plural(r.capOpen, 'finding')} outstanding`, detail: `FQM audit close-out at ${r.capPct}%.`, link: '/safety/cap?status=open', actors: SAFETY })
    if (r.compIssues > 0)
    ...
    // LOTO / tools (real)
    if (r.lotoOverdue > 0)
      push({ id: 'loto', severity: 'warning', icon: Lock, title: `${r.lotoOverdue} LOTO ${plural(r.lotoOverdue, 'point')} overdue`, detail: 'Isolation points unlabelled or past audit.', link: '/safety/loto?status=overdue', actors: WORKSHOP })
    if (r.toolsBad > 0)
      push({ id: 'tools', severity: 'warning', icon: Wrench, title: `${r.toolsBad} ${plural(r.toolsBad, 'tool')} defective or unsafe`, detail: 'Remove from service or repair.', link: '/safety/tools?condition=defective', actors: WORKSHOP })
    ...
    if (insp.overdue > 0)
      push({ id: 'insp', severity: insp.worstOver >= 7 ? 'critical' : 'warning', icon: ClipboardCheck, title: `${insp.overdue} ${plural(insp.overdue, 'bus', 'buses')} overdue for monthly inspection`, detail: insp.worstOver ? `Worst is ${insp.worstOver} ${plural(insp.worstOver, 'day')} overdue — assign a mechanic and inspect.` : 'Past due — assign a mechanic and inspect.', link: '/workshop/inspections?state=overdue', actors: [...OPS, 'workshop_supervisor'] })
    else if (insp.dueToday > 0)
      push({ id: 'insp', severity: 'warning', icon: ClipboardCheck, title: `${insp.dueToday} monthly ${plural(insp.dueToday, 'inspection')} due today`, detail: 'Scheduled for today — complete the inspection.', link: '/workshop/inspections?state=today', actors: [...OPS, 'workshop_supervisor'] })
    else if (insp.unscheduled > 0)
      push({ id: 'insp', severity: 'action', icon: ClipboardCheck, title: `${insp.unscheduled} ${plural(insp.unscheduled, 'bus', 'buses')} not scheduled for inspection`, detail: 'Assign a mechanic so every bus is inspected this month.', link: '/workshop/inspections?state=unscheduled', actors: [...OPS, 'workshop_supervisor'] })
