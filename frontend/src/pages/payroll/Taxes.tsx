    ...
    <div className="page space-y-5">
      ...
      <div className="card overflow-hidden">
        ...
        <div className="grid grid-cols-1 gap-3 p-4 sm:grid-cols-2">
          ...
          <label className="block"><span className="mb-1 block text-xs font-medium text-navy">NHIMA rate (% of basic)</span><input type="number" step="0.5" disabled={!canManage} className={inputCls} value={f.nhima_rate} onChange={(e) => set('nhima_rate', Number(e.target.value))} /></label>
      ...
      <p className="text-[11px] text-status-neutral">Defaults are Zambian figures (PAYE bands, NAPSA {DEFAULT_TAX.napsa_rate}% capped at {DEFAULT_TAX.currency} {DEFAULT_TAX.napsa_ceiling.toLocaleString()}, NHIMA {DEFAULT_TAX.nhima_rate}%). Verify against the current ZRA / NAPSA / NHIMA rates before running payroll.</p>
    </div>
  )
}
